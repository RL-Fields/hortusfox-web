				...
				@foreach ($plant_tasks as $task)
					@if (!$task->get('done'))
						<div class="task" id="task-item-{{ $task->get('id') }}">
							...
							<div class="task-description" id="task-item-description-{{ $task->get('id') }}"><pre>{!! ($task->get('description')) ? UtilsModule::purify(UtilsModule::translateURLs($task->get('description'))) : 'N/A' !!}</pre></div>
